    ...
    class ClaseGenerarRIDE
    {
        ...
        public bool generarRide(DataTable dtDatos, string filename, long iIdFactura)
        {
            try
            {
                ...
                if (bRespuesta == true)
                {
                    ...
                    {
                        dbPorcentajeDescuento = Convert.ToDouble(dtConsulta.Rows[0][2].ToString());

                        if ((dtConsulta.Rows[0][0].ToString() == "0") && (dtConsulta.Rows[0][1].ToString() == "0"))
                        {
                            llenarFacturaCompleta();
                        }

                        else if ((dtConsulta.Rows[0][0].ToString() == "1") && (dtConsulta.Rows[0][1].ToString() == "0"))
                        {
                            llenarFacturaConsumoAlimentos();
                        }

                        else if ((dtConsulta.Rows[0][0].ToString() == "1") && (dtConsulta.Rows[0][1].ToString() == "1"))
                        {
        ...
        public void llenarFacturaConsumoAlimentos()
        {
            try
            {
                ...
                for (int i = 0; i < dtDatos.Rows.Count; i++)
                {
                    dPrecioUnitario = dPrecioUnitario + Convert.ToDouble(dtDatos.Rows[i][26].ToString());
                    dValorDescuento = dValorDescuento + Convert.ToDouble(dtDatos.Rows[i][27].ToString());
                    dValorIce = dValorIce + Convert.ToDouble(dtDatos.Rows[i][28].ToString());
                    dValorIva = dValorIva + Convert.ToDouble(dtDatos.Rows[i][31].ToString());
                }
                ...
                iColumna++;
                dr["propina"] = dtDatos.Rows[0][iColumna].ToString();
                iColumna++;
