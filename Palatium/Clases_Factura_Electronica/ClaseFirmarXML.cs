        ...
        public void Gsub_trae_parametros_certificado(string[] P_St_certificado_digital)
        {
            try
            {
                ...
                if (bRespuesta == true)
                {
                    if (dtConsulta.Rows.Count > 0)
                    {
                        P_St_certificado_digital[0]  = conexion.GFun_Va_Valor_Defecto(dtConsulta.Rows[0].ItemArray[0].ToString(), "");
                        P_St_certificado_digital[1] = conexion.GFun_Va_Valor_Defecto(dtConsulta.Rows[0].ItemArray[1].ToString(), "");
                    }
                }
